...
class Type3E:
    ...
    def _encode_value(self, value, mode="short", isSigned=True):
    ...
    def batchread_wordunits(self, headdevice, readsize):
        ...
        request_data += self._encode_value(readsize,mode="long")
        ...
        for _ in range(readsize):
            wordvalue = self._decode_value(recv_data[data_index:data_index+self._wordsize], mode="long", isSigned=False)
    ...
    def batchwrite_wordunits(self, headdevice, values):
        ...
        request_data += self._make_commanddata(command, subcommand)
        request_data += self._make_devicedata(headdevice)
        request_data += self._encode_value(write_size,mode="long")
        for value in values:
            request_data += self._encode_value(value, isSigned=False)
        send_data = self._make_senddata(request_data)
